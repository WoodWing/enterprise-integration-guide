...
Since Enterprise 8, logging can be enabled per client IP, and for each client IP a separate logging folder is created. (See DEBUGLEVELS option described in the Help Center how to configure.) You can add your client IP with a DEBUG value so that only for your client application all details are logged.
...
1. Have a look in the log folder. The Web Service requests and responses are logged, all in separate files. Request files have a “\_Req” postfix and responses a “\_Resp” postfix.
...
> **\[Since 8.0\]** For SOAP and JSON the original request is logged. Because AMF is a binary format, this is not done for AMF. In such cases you have to rely on the PHP objects logged in TXT format; this can be enabled with the LOG\_INTERNAL\_SERVICES option.
...
1. Determine the used **URN** `urn:SmartConnection` and look up the WSDL file (by using the table discussed in the previous chapter). In this example, you will find the SCEnterprise.wsdl file.
1. Determine the **operation** `<LogOnResponse>` (fired by the client) and use this as a starting point to look up the **parameters** `<Ticket>`, `<Publications>`, `...` and **data structure** `<PublicationInfo>` in the WSDL (as looked up). This is explained below in more detail.
