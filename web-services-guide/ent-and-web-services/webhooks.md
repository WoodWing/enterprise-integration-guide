...
---
The Webhooks feature has been introduced to enable 3rd parties acting on events that take place in the Studio system.
...
## How it works
Some web services have been added to Studio Server which allows a 3rd party to register a URL to be called when a certain event takes place in Studio. For each webhook registration, Studio Server creates a record in the database, and creates a dedicated message queue in RabbitMQ. 
...
When a Studio user performs an operation that triggers such event, Studio Server makes a snapshot of the data involved and pushes it into the Event Bus. (The Event Bus is another message queue in RabbitMQ that is created by Studio Server.)

A scheduler (crontab, Task Scheduler, Lambda, etc) periodically calls Studio Server to dispatch the events from the Event Bus message queue to the Webhook registration message queues. Another scheduler periodically calls Studio Server to read events from the Webhook queues and to call the 3rd party URL, providing the event data. It secures the call by adding a signature that is based on the event data and the secret.

The 3rd party receives the Webhook request of Studio Server. It reads the event data and signature. It composes a signature based on the request data and the secret. Only if the received and composed signatures are the same, the 3rd party accepts the Webhook request.
 ...
 * Add a row to a Google Sheet for each article created in Studio.
 * Update a record in an external MySQL database to keep track of some properties that were modified for a layout in Studio.
 * Download the file and upload it to a 3rd party image processor for each image created in Studio.
 * ... etc 

To disable the integration, the 3rd party calls web services to remove the registration(s) at Studio Server. That stops Studio Server from calling the registered URL(s).
...
* The user gets back a response from Studio Server and continues working, performing subsequential workflow operations. 
* In the background, the event processors of Studio Server picks up the event data and calls the registered Webhooks. The called 3rd party may also perform subsequential workflow operations.

In short, the user and the 3rd party integration operate asynchronously (in parallel). This is unlike the basic nature of Studio where almost everything for one request gets processed synchronously (sequential). In an asynchronous context it cannot be assumed that certain content is still there, or still the latest version, or still in a locked or unlocked status, etc. Things may have changed in the meantime. The Event Bus may be a bit behind, or the 3rd party may went offline for a moment, which may worsen this effect. The 3rd party integration must be aware of this and should take the responsibility to deal with various race conditions and unexpected status of content.

Most advisable for a Webhooks based integration is:
 * To base its reaction on the event data itself; Without the need to query Studio Server to enrich the data and without the need to update information in Studio Server that is related to the subject (workflow object) of the event. Such integration will never suffer from the complexity introduced by the asynchronous nature of events.
 * Not to call Studio Server web services. If each event triggers an integration that calls Studio Server, the workflow traffic would duplicate. If this is really needed, try to make it happen less often, such as only for a certain brand, category, status, content type, event type, etc. 
...
Due to performance differences, a certain workflow event may get processed on a different moment for one registration compared to another. In other words, two registration should not expect to get called on the very time moment (for a certain workflow event).
...
| Moved to Trash Can | `com.woodwing.studio/object/deleted` | Moving an object from workflow to the Trash Can.
| Deleted | `com.woodwing.studio/object/deleted-permanently` | Deleting an object permanently (from workflow or Trash Can).
| Restored | `com.woodwing.studio/object/restored` | Restoring a deleted object from Trash Can (back into the workflow).
| Copied | `com.woodwing.studio/object/copied` | Creating a new object by copying from an existing object.
...
* Change object targets or object relations without changing properties (e.g. drag & drop an object into a dossier).
* Send messages or adding operations to objects.
* Modifications to planned pages or adverts.

## RabbitMQ instances
Studio Server integrates with RabbitMQ to implement two features. Each feature can be enabled or disabled independently. Both features can be hosted in one RabbitMQ instance or each feature can be hosted a separated RabbitMQ instance. 
...
# Configuration
This chapter describes how the Webhooks feature can be setup.
...
Tip: If you want to use one RabbitMQ installation to host both features, you can simply make the Event Bus options inherit from the Message Queue options by adding them like this to your `.../StudioServer/config/config_overrule.php` file:
```php
...
The event data pushed into the Event Bus message queue of RabbitMQ must be processed; The events must be dispatched to those Webhook queues for which a Webhook registration has been setup.

Subsequentially the event data pushed into the Webhook queues must be processed too; The registered Webhook URL must be called to notify the 3rd party about the event. 

In other words, there are two processors required. A scheduler tool can be used to periodically call the processors. You can use e.g. the Task Scheduler for Windows, crontab for Linux or a Lambda for AWS. The processors must be called over HTTP for which you can use the cURL commandline tool.
...
# Integration
This chapter describes how a 3rd party can be integrated with Studio by using the Webhooks feature.
...
Logon to Studio Server with system administration credentials to obtain a valid `Ticket`.
...
Logout from Studio Server.

## How to accept a Webhook request
 ...
 * hasn't been modified underway (man-in-the-middle attack).

This can be done as follows:
1. Check whether it is an HTTP POST request. Deny access for other HTTP methods. 
1. Check whether it is an HTTPS request. Deny access for non-SSL encrypted request.
...
## How to parse Webhook event data
...
Inside the `data` element there is an `Object` for which `Metadata` and `Targets` are provided. The structure of those elements is defined in the workflow service definition (SCEnterprise.wsdl file).

## How to disable or remove a Webhook registration
...
Keep in mind of the race condition of having events in the queue that serves the Webhook registration being removed. While removing:
